...
from layout_class import Layout


def run_game(width: int, height: int, display_surface: pygame.Surface):
    clock = pygame.time.Clock()

    palace = pygame.image.load("palace.png").convert_alpha()
    ...
    you_win_im = pygame.image.load("you_win.png").convert_alpha()
    ...
    layout = Layout()
    layout = layout.populate()

    
    for i in range(len(layout['Obstacles'])):
        obstacles.append([Obstacle(),layout['Obstacles'][i], 0])
    
    
    for i in range(len(layout['Bombs'])):
        bombs.append([Bomb(), layout['Bombs'][i][0], layout['Bombs'][i][1], -1])#random()*(sand_height-200), -1])

    
    for i in range(len(layout['Mines'])):
        mines.append([Mine(), layout['Mines'][i][0], layout['Mines'][i][1],-1])#(sand_height+100)+random()*300, -1])
    
    
    for i in range(len(layout['Vessels'])):
        vessels.append([Vessel(), layout['Vessels'][i]])
    ...
    while intro:
        ...
        tick = pygame.time.get_ticks() - start_tick
    ...
    while game_won:
        ...
        if t_end>=1:
            character_y = sand_height
            character_x = worm_head_x - worm_width/2 + width*c_end
